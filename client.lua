...
Citizen.CreateThread(function() -- Creation des blips sur la map
...
if enableBankingGui then -- Thread principal
  ...
  Citizen.CreateThread(function()
    ...
    while true do
      ...
      Citizen.Wait(0)
      TriggerEvent("pNotify:SendQueueMax", "notif", 20)

      if(IsNearBank() or IsNearATM()) then

          TriggerEvent('esx:showNotification', "Appuyez sur ~g~[E] ~w~pour acceder à la banque")
        ...
        if IsControlJustPressed(1, 38)  then
          ...
          if (IsInVehicle()) then
            ...
            TriggerEvent("pNotify:SendNotification", {
              text = "Vous ne pouvez pas acceder a la banque dans un vehicule",
              type = "error",
              queue = "notif",
              timeout = 5000,
              layout = "bottomRight"
...
Citizen.CreateThread(function() --Thread qui empeche les mouvements in game lors de l'utilisation de la banque
  while true do
...
AddEventHandler('bank:deposit', function(amount)
  if(IsNearBank() == true or depositAtATM == true and IsNearATM() == true or depositAnywhere == true ) then
    if (IsInVehicle()) then
      TriggerEvent("pNotify:SendNotification", {
              text = "Vous ne pouvez pas acceder à l'ATM dans un vehicule",
              type = "error",
              queue = "notif",
              timeout = 5000,
              layout = "bottomRight"
    ...
    TriggerEvent("pNotify:SendNotification", {
              text = "Les dépots se font uniquement en banque",
              type = "error",
              queue = "notif",
              timeout = 5000,
              layout = "bottomRight"
...
AddEventHandler('bank:withdraw', function(amount)
  if(IsNearATM() == true or IsNearBank() == true or withdrawAnywhere == true) then
    ...
    TriggerEvent("pNotify:SendNotification", {
              text = "Ceci n'est pas une banque ou un ATM",
              type = "error",
              queue = "notif",
              timeout = 5000,
              layout = "bottomRight"
...
AddEventHandler('bank:transfer', function(toPlayer, amount)
  ...
  if (playing ~= false) then
    ...
    TriggerEvent("pNotify:SendNotification", {
        text = "Cette personne n'est pas en ville",
        type = "error",
        queue = "notif",
        timeout = 5000,
        layout = "bottomRight"
